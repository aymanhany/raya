...
class _OrderConfirmationState extends State<OrderConfirmation> {
                    ...
                    Container(
                      child: Row(
                        children: [
                          Expanded(
                            child: Image.network(
                              'https://raya.akwadweb.com/storage/carts/${widget.carData.id}/${widget.carData.pics.first}',
                              fit: BoxFit.cover,
                            ),
                          ),
                          Expanded(
  ...
  Future subbmitOrder(data, paymentMethod, user, color) async {
    var colorString = color.toString();
    final response = await http.post(
      Uri.parse('https://Httpraya-backend.com/api/orders'),
      body: {
        "lang": "en",
        "type": "buy",
        "cart_id": "${data.id}",
    ...
    if (response.statusCode == 200) {
      ...
      final crmAddDeal = await http.post(
        Uri.parse(
            'https://rayaauto.bitrix24.com/rest/8750/nfbbkk99zdgm037d/crm.deal.add.json?fields[TITLE]=Mobile App Buy / Cart = ${data.name}} / ${user.name} / ${user.mobile} / ${user.email} / ${colorString} / ${paymentMethod}&fields[UF_CRM_1621838721]=127&fields[CATEGORY_ID]=0'),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
