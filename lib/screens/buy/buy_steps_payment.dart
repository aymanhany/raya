...
import 'package:raya/screens/buy/visa.dart';
import 'dart:convert';
import 'dart:async';
...
class _BuyStepsPaymentState extends State<BuyStepsPayment> {
              ...
              Column(
                children: [
                  Container(
                    ...
                    child: TextButton(
                      onPressed: () {
                        ...
                        var iframe = 380572;
                        var intid = 2253586;
                        getVisaApi(carData.price, user, iframe, intid);
                      },
                      child: Text(
                  ...
                  Container(
                    ...
                    child: TextButton(
                      onPressed: () {
                        Provider.of<PaymentsProvider>(context, listen: false).paymentMethod = 'Bank Installment';
                        var iframe = 380571;
                        var intid = 2279159;
                        getVisaApi(carData.price, user, iframe, intid);
                      },
                      child: Text(
                        'Bank Installment '.toUpperCase(),
                        style: TextStyle(color: kPrimaryColor),
                      ),
                    ),
                  ),
  ...
  Future getVisaApi(price, user, iframe, intid) async {
    var authToken;
    var orderId;

    var amount = int.parse(price.replaceAll(',', '')) * 100;
    // First Request
